...
public class SymptomInfo extends AppCompatActivity {
    ...
    public void give_symptom(String ID,String date, String []a)
    {
        ...
        try {
            ...
            String result = task.execute(id,date, cramps, breastTenderness,
                    headache, acne, lumbago, nausea,
                    fatigue, abdominalBloating, desires,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        symptoms1.setChecked(Boolean.parseBoolean(init_info[0]));
        symptoms2.setChecked(Boolean.parseBoolean(init_info[1]));
        symptoms3.setChecked(Boolean.parseBoolean(init_info[2]));
        symptoms4.setChecked(Boolean.parseBoolean(init_info[3]));
        symptoms5.setChecked(Boolean.parseBoolean(init_info[4]));
        symptoms6.setChecked(Boolean.parseBoolean(init_info[5]));
        symptoms7.setChecked(Boolean.parseBoolean(init_info[6]));
        symptoms8.setChecked(Boolean.parseBoolean(init_info[7]));
        symptoms9.setChecked(Boolean.parseBoolean(init_info[8]));
        symptoms10.setChecked(Boolean.parseBoolean(init_info[9]));
        symptoms11.setChecked(Boolean.parseBoolean(init_info[10]));
        symptoms12.setChecked(Boolean.parseBoolean(init_info[11]));
        ...
        for(int i=0;i<12;i++)
        {
            if ((symptom_Check[i].equals( "false") )&& (init_info[i].equals("true")))
                symptom_Check[i] = "true";
            else if ((symptom_Check[i] .equals("true")) && (init_info[i].equals("false")))
                symptom_Check[i] = "true";
            else if ((symptom_Check[i].equals( "false")) && (init_info[i].equals("false")))
                symptom_Check[i] = "false";
            else if ((symptom_Check[i].equals( "true")) && (init_info[i].equals("true")))
                symptom_Check[i] = "false";
    ...
    class setSymptom extends AsyncTask<String,Void,String> {
        ...
        protected String doInBackground(String... strings) {
            try {
                ...
                OutputStreamWriter osw = new OutputStreamWriter(conn.getOutputStream());
                sendMsg = "id="+strings[0]+"&date="+strings[1]+
                        "&cramps="+strings[2]+"&breastTenderness="+strings[3]+"&headache="+strings[4]
                        +"&acne="+strings[5]+"&lumbago="+strings[6]+"&nausea="+strings[7]+"&fatigue="+strings[8]
                        +"&abdominalBloating="+strings[9]+"&desires="+strings[10]+"&insomnia="
                        +strings[11]+"&constipation="+strings[12]+"&diarrhea="+strings[13];
                // GET방식으로 작성해 POST로 보냄 ex) "id=admin&pwd=1234";
                osw.write(sendMsg);                           // OutputStreamWriter에 담아 전송
                osw.flush();
